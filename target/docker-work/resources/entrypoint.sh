...
jvm_mem=${JAVA_MEM:-" "}
jvm_opts=${JAVA_OPTS:-"-server -XX:HeapDumpPath=/var/log/ocpp16j/heap-dump-%p.hprof"}
arguments=${ARGUMENTS:-" --package.name=ocpp16j --package.directory=ocpp16j"}
...
mkdir -p /var/log/ocpp16j; echo "heap dumps  /var/log/ocpp16j/heap-dump-<pid>.hprof"

java ${jvm_opts} ${jvm_gc} ${jvm_mem} ${proxy_params}  ${otel_agent_attach} -jar /data/ocpp16j.jar ${arguments}
